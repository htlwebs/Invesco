...
/**
 * The Client class handles requests to Recast.AI API. Note that requests
 * methods and stopRecording should not be called in the main thread of your
 * application because they process http requests.
 *
 */
public class Client {
	 ...
	 * 
	 * @param token
	 *            The token to authenticate to Recast.AI
	 */
	 ...
	 * 
	 * @param language
	 *            The language for the language processing
	 */
	public void setLanguage(String language) {
		this.language = language;
	}

	// TODO Changing to java folder
	...
	/**
	 * Starts recording audio from the microphone. Note that the audio must be
	 * shorter than 10 seconds to be processed by Recast.AI
	 * 
	 * @throws RecastException
	 *             if the client is already recording (the recording will stop)
	 */
	...
	/**
	 * Stops recording from the microphone and returns the Response
	 * corresponding to the audio input after beeing processed
	 * 
	 * @return A Recast.AI Response
	 * @throws RecastException
	 *             if the Client is not recording of if an error occurs (invalid
	 *             audio...)
	 * @see Response
	 */
	 ...
	 * @param options
	 *            A map of parameters to the request. Parameters can be "token"
	 *            and "language"
	 * @return The Response corresponding to the input
	 * @throws RecastException
	 *             if Recast.AI can't process the text
	 * @see Response
	 */
	...
	private String sendAudioFile(String name, String token, String language)
			throws RecastException {
	//	String recastJson = "";
		StringBuilder sb;
		try {
	...
	/**
	 * Performs a voice file request to Recast.AI. Note that the audio must not
	 * exceed 10 seconds and be in wav format.
	 * 
	 * @param filename
	 *            The name of the file
	 * @return The Response corresponding to your input
	 * @throws RecastException
	 *             if the file is invalid or Recast.Ai can't process the file
	 */
	 ...
	 *            The name of the file
	 * @param options
	 *            A map of parameters for the request. This map can contains
	 *            "token" and/or "language".If a parameter is not provided, the
	 *            request will use the token or language of the Client. I it has
	 *            not language, the language used will be the default language
	 *            of the corresponding bot
	 * @return The Response corresponding to your input
	 * @throws RecastException
	 *             if the file is invalid or Recast.Ai can't process the file
	 */
	public Response fileRequest(String filename, Map<String, String> options)
			throws RecastException {
		String token;
		String language;
	...
	public String doApiRequest(String text, String token, String language)
			throws RecastException {
		URL obj;
		...
		if (responseCode == HttpsURLConnection.HTTP_OK) {
			try {
				...
				throw new RecastException(
						"Unable to read response from Recast", e);
			}
